...
<body class="bg-olive-50 text-olive-900 font-sans antialiased lg:h-screen lg:overflow-hidden flex flex-col">
  ...
  <main class="flex-grow flex flex-col lg:flex-row overflow-visible lg:overflow-hidden no-print">
    ...
    <div class="w-full lg:w-5/12 bg-white border-b lg:border-b-0 lg:border-r border-olive-100 overflow-visible lg:overflow-y-auto custom-scroll pb-24 lg:pb-6 relative">
        <div class="p-4 lg:p-6 space-y-6">
            ...
            <div class="flex justify-between items-center bg-stone-50 p-3 rounded-xl border border-stone-200 shadow-inner">
                <div class="flex items-center gap-2 text-stone-600">
                    <i data-lucide="save" class="w-4 h-4"></i>
                    <span class="text-xs font-bold uppercase tracking-wide">Rascunho Local</span>
                </div>
                <div class="flex gap-2">
                    <label class="cursor-pointer bg-white border border-stone-300 text-stone-700 px-3 py-1.5 rounded-lg hover:bg-stone-100 transition text-xs font-bold flex items-center gap-1">
                        <i data-lucide="folder-open" class="w-3 h-3"></i> Abrir
                        <input type="file" id="loadDraft" accept=".json" class="hidden" onchange="loadProgress(event)">
                    </label>
                    <button onclick="saveProgress()" class="bg-olive-800 text-white px-3 py-1.5 rounded-lg hover:bg-olive-900 transition text-xs font-bold flex items-center gap-1 shadow">
                        <i data-lucide="downloadCloud" class="w-3 h-3"></i> Salvar
                    </button>
                </div>
            </div>
            ...
            <div class="space-y-3 bg-orange-50/50 p-4 rounded-xl border border-orange-100">
                <h3 class="font-bold text-orange-800 text-xs uppercase tracking-wide flex items-center gap-2"><i data-lucide="wallet" class="w-4 h-4"></i> Condições de Pagamento (%)</h3>
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="text-[10px] font-bold text-orange-600">Entrada (%)</label>
                        <input type="number" id="entryPct" class="w-full p-3 border border-orange-200 rounded-xl text-base calc-input bg-white text-orange-800 font-bold" value="60">
                    </div>
                    <div>
                        <label class="text-[10px] font-bold text-orange-600">Pagamento Final (%)</label>
                        <input type="number" id="balancePct" class="w-full p-3 border border-orange-200 rounded-xl text-base calc-input bg-white text-orange-800 font-bold" value="40">
                    </div>
                    <div class="col-span-2 pt-2 border-t border-orange-200/50 flex items-center gap-2">
                        <input type="checkbox" id="showCreditCard" class="w-5 h-5 text-orange-600 rounded border-orange-300 focus:ring-orange-500" checked onchange="calculate()">
                        <label for="showCreditCard" class="text-xs font-bold text-orange-900 cursor-pointer">Exibir opções de parcelamento no cartão para o cliente</label>
                    </div>
                </div>
            </div>
        ...
        <div class="fixed lg:static bottom-0 left-0 w-full bg-gray-900 text-white p-4 lg:rounded-xl shadow-[0_-5px_15px_rgba(0,0,0,0.2)] lg:shadow-lg z-40 lg:mb-6">
            <div class="flex justify-between items-end">
                <div>
                    ...
                    <span class="text-xs text-gray-400"><span id="sugEntryLabel">Entrada:</span> <span id="sugEntry" class="text-white font-bold">R$ 0,00</span></span>
                </div>
                    ...
                    <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
                        ...
                        <div class="space-y-3 text-sm border-t border-gray-200 pt-4">
                            <div class="flex justify-between items-center">
                                <span class="font-bold text-olive-900" id="propEntryLabel">1. Entrada (60%) - Início Fabricação</span>
                                <span class="font-bold text-olive-900" id="propEntry">R$ 0,00</span>
                            </div>
                            <div class="flex justify-between items-center text-gray-600">
                                <span id="propBalanceLabel">2. Entrega (40%) - Final Instalação</span>
                                <span id="propBalance">R$ 0,00</span>
                            </div>
                        </div>

                        <div id="creditCardSection" class="mt-6 pt-4 border-t border-gray-200">
  ...
  <script>
    ...
    function calculate() {
        ...
        const entryPct = getVal('entryPct') || 60;
        const balancePct = getVal('balancePct') || 40;
        ...
        const entryVal = finalPrice * (entryPct / 100);
        const balanceVal = finalPrice * (balancePct / 100);
        ...
        document.getElementById('sugEntryLabel').innerText = `Entrada ${entryPct}%:`;
        document.getElementById('sugEntry').innerText = fmt(entryVal);

        document.getElementById('propEntryLabel').innerText = `1. Entrada (${entryPct}%) - Início Fabricação`;
        document.getElementById('propBalanceLabel').innerText = `2. Entrega (${balancePct}%) - Final Instalação`;

        document.getElementById('propTotalValue').innerText = fmt(finalPrice);
        document.getElementById('propEntry').innerText = fmt(entryVal);
        document.getElementById('propBalance').innerText = fmt(balanceVal);
        ...
        // Verifica o toggle do cartão
        const showCC = document.getElementById('showCreditCard').checked;
        const ccSection = document.getElementById('creditCardSection');
        if(showCC && finalPrice > 0) {
    ...
    function saveProgress() {
        const data = {
            ...
            costMaterial: document.getElementById('costMaterial').value,
            costLabor: document.getElementById('costLabor').value,
            costFreight: document.getElementById('costFreight').value,
            costAssembler: document.getElementById('costAssembler').value,
            costFat: document.getElementById('costFat').value,
            costMarketing: document.getElementById('costMarketing').value,
            commWarlyn: document.getElementById('commWarlyn').value,
            commPedro: document.getElementById('commPedro').value,
            taxPct: document.getElementById('taxPct').value,
            spacePct: document.getElementById('spacePct').value,
            entryPct: document.getElementById('entryPct').value,
            balancePct: document.getElementById('balancePct').value,
            discountPct: document.getElementById('discountPct').value,
            discountDesc: document.getElementById('discountDesc').value,
            descriptionInput: document.getElementById('descriptionInput').value,
            showCreditCard: document.getElementById('showCreditCard').checked
        };
        ...
        const safeName = (data.clientName || "Rascunho").replace(/[^a-z0-9]/gi, '_').toLowerCase();
        dlAnchorElem.setAttribute("download", `orcamento_${safeName}.json`);
    ...
    function loadProgress(event) {
        ...
        reader.onload = function(e) {
            try {
                ...
                calculate();
                updateProposal();
                alert("Rascunho carregado com sucesso! (Lembre-se de reanexar as imagens, se houver).");
            } catch (err) {
                alert("Erro ao ler o arquivo. Certifique-se de que é um rascunho válido (.json).");
            }
        };
        reader.readAsText(file);
        event.target.value = ""; // Reseta o input para permitir carregar o mesmo arquivo denovo
    }
